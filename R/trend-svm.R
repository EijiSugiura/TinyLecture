...
## SVM
y <- as.data.table(z %>% select(-Hostname))
y[is.na(y)] <- 0
...
svm1.table <- y.table
svm1.error <- 1-sum(diag(y.table))/sum(y.table)
...
y.table <- table(y.pred, y.test$Reboot)
svm2.table <- y.table
svm2.error <- 1-sum(diag(y.table))/sum(y.table)
...
y.table <- table(y.pred, y.test$Reboot)
svm3.table <- y.table
svm3.error <- 1-sum(diag(y.table))/sum(y.table)
